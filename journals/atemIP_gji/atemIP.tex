...
% Seogi: sure
% ** can you fix references to Oldenburg to omit the W.** 
...
%Units of the recovered pseudo-chargeability could be various depending on various types of the IP data.



 %Definition of the chargeability, $\eta$ can be expressed as $\eta = \frac{\siginf-\sigma_0}{\siginf}$, where $\siginf$ and $\sigma_0$ are conductivity at the infinite frequency and the zero frequency, respectively. Considering that we cannot measure EM responses at both zero and infinite frequencies, a pseudo-chargeability recovered from a conventional IP inversion is different from the chargeability hence we call recovered values from the IP inversion as the pseudo-chargeability.  
...
% which provides a reasonable assumption $\siginf \simeq$  0 S/m except for chargeable rocks. This set-up makes the first term ignorable especially for outside of chargeable rocks. However, if the conductivity of the chargeable rocks is similar to that of the background rock the first term could be important especially outside of the chargeable rocks. 
...
% In contrast, if we invert DC data (assuming a galvanic source), recovered conductivity will be closer to $\sigma_0$ (DC conductivity).
...
%( This parallels \cite[]{Yuval1997,Hordt2006}. )
